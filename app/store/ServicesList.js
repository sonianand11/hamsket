Ext.define('Rambox.store.ServicesList', {
	...
	,data: [
		...
		},
		{
			...
			,type: 'email'
			,manual_notifications: true
			,js_unread: `let checkUnread=()=>{const fav=$(".ms-FocusZone [role=tree]:first i[data-icon-name=Inbox]").siblings()[1],folders=$(".ms-FocusZone [role=tree]:nth(1)")[0].children[1].querySelector("span span"),innerText=fav?fav.innerText:folders?folders.innerText:0,i=parseInt(innerText,10)||0;rambox.updateBadge(i)};setInterval(checkUnread,3e3);`
		},
		{
			 id: 'outlook365'
			,logo: 'outlook365.png'
